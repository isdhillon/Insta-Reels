...
import { Avatar,Card,CardMedia,CardContent,Divider,TextField } from '@material-ui/core'
...
export default function Feed() {
    ...
    function handleInputFile(e){
        ...
        if(file!=null){
            try{
                setLoading(true);
                let puid=uuid();
                const uploadTaskListener=storage.ref(`/reels/${puid}`).put(file)
    ...
    useEffect(()=>{
        ...
        let observer=new IntersectionObserver(callBack,condtionObject)
    ...
    return (
        pageLoading?<div><LinearProgress color="secondary"></LinearProgress></div>:
        <div>
            <div className="navBar" key="navBar">
                <AppBar position="static" style={{backgroundColor:"white"}}>
                    ...
                    <IconButton><ExploreIcon></ExploreIcon></IconButton>
            ...
            <div className="upload-reels" style={{display:"flex",alignItems:"center",justifyContent:"center" ,marginBottom:"1rem"}}>
                ...
                <input type="file" accept="video/*" onChange={(e)=>{handleInputFile(e)}} hidden>
                </input>
                </Button>
            </div>
            <div className={classes.alignCenter} key="uploadbtn">
                ...
                {isCommentModalVisible==true?<div className={classes.commentModal} tabIndex="0" onKeyDown={(e)=>{if(e.key=="Escape")setCommentModalVisibility(false)}}>
                    <div style={{display:'flex'}} className={classes.fullHeight}>
                        <Card key="reel">
                            <CardMedia>
                                <video muted={true} autoPlay style={{height:"80vh"}}>
                                    <source src={commentReel.videoUrl}>
                                    </source>
                                </video>
                            </CardMedia>
                        </Card>
